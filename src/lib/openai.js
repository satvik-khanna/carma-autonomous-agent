  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are Carma, an expert automotive AI agent. You analyze car listings to find the perfect match for a user based on their lifestyle, budget, and needs. You provide cutting-edge, data-driven recommendations.
      ...
      .map((car, index) => {
        ...
        return {
          ...
          overallScore: clampScore(
            ranking.overallScore || ranking.overall_score,
          ),
...
- "recommendation": "buy" | "rent" | "consider" (return the best tag to highlight the car)
...
function getDefaultScores() {
  return {
    ...
    aiExplanation:
      "Unable to generate AI analysis. Please review this listing manually.",
  };
}
...
function applyFallbackScoring(cars, preferences) {
  ...
  return cars
    .map((car) => {
      ...
      const overallScore = Math.round((valueScore + matchScore) / 2);
      ...
      return {
        ...
        recommendation: valueScore >= 7 && matchScore >= 7 ? "buy" : "consider",
        aiExplanation:
          "Scored using basic analysis. Connect to OpenAI for detailed recommendations.",
      };
    })
