...
MILES_RE  = re.compile(r"\b(\d{1,3}(?:,\d{3})+|\d{3,})\s*(?:mi|miles)\b", re.IGNORECASE)
MILEAGE_LABEL_RE = re.compile(r"(?i)\b(?:mileage|miles?)\s*:\s*([0-9]{1,3}(?:,[0-9]{3})*[0-9]*)")
...
ATTR_LINE_RE = re.compile(
    r"(?im)^\s*(" + "|".join(re.escape(k) for k in CL_ATTR_KEYS) + r"):\s*(.*?)\s*$"
)
ATTR_INLINE_RE = re.compile(
    r"(?:^|(?<=\s))(" + "|".join(re.escape(k) for k in CL_ATTR_KEYS) + r"):\s*",
    re.IGNORECASE,
)
ATTR_VALUE_STOPS = [
...
DEALER_BOLD_RE = re.compile(r"\*\*([^*:]{2,30}?)(?::\*\*|\*\*:)\s*(.*?)(?:\n|$)")
...
NL_PATTERNS: Dict[str, re.Pattern] = {
    "transmission": re.compile(r"(?i)\b(automatic|manual|cvt|6[\s-]?speed|5[\s-]?speed|4[\s-]?speed)\b"),
    "title status": re.compile(r"(?i)\b(clean\s+title|salvage\s+title|rebuilt\s+title|branded\s+title)\b"),
    ...
    "fuel":  re.compile(r"(?i)\b(gas(?:oline)?|diesel|hybrid|electric|plug[\s-]?in)\b"),
    "drive": re.compile(
        r"(?i)\b(fwd|awd|4wd|rwd|2wd"
        r"|front[\s-]?wheel[\s-]?drive|all[\s-]?wheel[\s-]?drive"
        r"|four[\s-]?wheel[\s-]?drive|rear[\s-]?wheel[\s-]?drive)\b"
    ),
    "type":  re.compile(r"(?i)\b(sedan|coupe|suv|hatchback|wagon|convertible)\b"),
}
...
POSTED_RE  = re.compile(r"(?i)(?:^|\n)\s*posted\s*[:\n]?\s*(20\d{2}-\d{2}-\d{2}[\sT]?\d{2}:\d{2})")
UPDATED_RE = re.compile(r"(?i)(?:^|\n)\s*updated\s*[:\n]?\s*(20\d{2}-\d{2}-\d{2}[\sT]?\d{2}:\d{2})")
GMAP_RE    = re.compile(r"https?://www\.google\.com/maps/search/([-\d.]+),([-\d.]+)")
...
def choose_text(rec: Dict[str, Any]) -> str:
    ...
    if "<" in text and ">" in text:
        real_tags = len(re.findall(
            ...
            r"|script|style|link|meta)\b", text, re.I,
        ))
        if real_tags >= 3:
...
def _parse_cl_attrs(text: str) -> Dict[str, str]:
    ...
    for i, m in enumerate(matches):
        key = m.group(1).strip().lower()
        val_start = m.end()
        val_end   = (matches[i + 1].start() if i + 1 < len(matches)
...
def norm_transmission(val: Optional[str]) -> Optional[str]:
    ...
    if v in ("automatic", "auto"): return "automatic"
    if v in ("manual", "standard", "stick"): return "manual"
    if "cvt" in v or "continuously" in v: return "cvt"
    m = re.match(r"(\d)[\s-]?speed", v)
    if m: return f"{m.group(1)}-speed"
...
def norm_fuel(val: Optional[str]) -> Optional[str]:
    ...
    if v in ("gas", "gasoline"): return "gas"
    if "diesel" in v: return "diesel"
    if "hybrid" in v or "plug" in v: return "hybrid"
    if "electric" in v: return "electric"
    if v in ("other", "flex"): return v
    return None

def norm_drive(val: Optional[str]) -> Optional[str]:
    if not val: return None
    v = clean_dealer_val(val).lower().replace("-", " ").strip()
    if v == "fwd" or "front wheel" in v: return "fwd"
    if v == "awd" or "all wheel" in v:   return "awd"
    if v in ("4wd", "4x4") or "four wheel" in v: return "4wd"
    if v == "rwd" or "rear wheel" in v:  return "rwd"
    if v == "2wd": return "2wd"
    return None
...
def norm_body_type(val: Optional[str]) -> Optional[str]:
    if not val: return None
    v = clean_dealer_val(val).lower().strip()
    if "sedan" in v or "4d" in v or "4 d" in v: return "sedan"
    if "coupe" in v or "2d" in v or "2 d" in v: return "coupe"
    if "suv"   in v: return "suv"
    if "hatch" in v: return "hatchback"
    if "wagon" in v: return "wagon"
    if "convert" in v: return "convertible"
    if "pickup" in v or "truck" in v: return "truck"
    if "van"   in v or "mini" in v: return "van"
    if v == "other": return "other"
    return None
...
def norm_condition(val: Optional[str]) -> Optional[str]:
    ...
    for key, canon in canonical.items():
        if key in v: return canon
    return v

def norm_used_new(condition_raw: Optional[str]) -> Optional[str]:
    if not condition_raw: return None
    return "new" if condition_raw == "new" else "used"

def norm_title_status(val: Optional[str]) -> Optional[str]:
    if not val: return None
    v = clean_dealer_val(val).lower().strip()
    if "clean"   in v: return "clean"
    if "salvage" in v: return "salvage"
    if "rebuilt" in v: return "rebuilt"
    if "branded" in v: return "branded"
    if "lien"    in v: return "lien"
    if "missing" in v: return "missing"
    return v
...
def norm_cylinders(val: Optional[str]) -> Optional[str]:
    ...
    if m: return f"{m.group(1)} cylinders"
    m = re.search(r"v[\s-]?(\d+)", v)
    if m: return f"{m.group(1)} cylinders"
    m = re.search(r"i[\s-]?(\d+)|inline[\s-]?(\d+)", v)
    if m: return f"{(m.group(1) or m.group(2))} cylinders"
    m = re.search(r"(\d\.\d)\s*l", v)
    if m: return f"{m.group(1)}L"
    if re.match(r"^\d$", v): return f"{v} cylinders"
    m = re.match(r"^(\d\.\d)$", v)
    if m: return f"{m.group(1)}L"
    return None
...
def norm_paint_color(val: Optional[str]) -> Optional[str]:
    ...
    color_map = {
        ...
        "gray": "gray",   "grey": "gray",   "red": "red",
        "blue": "blue",   "green": "green", "brown": "brown",
        "gold": "gold",   "beige": "beige", "orange": "orange",
        "purple": "purple", "tan": "tan",   "maroon": "maroon",
        "burgundy": "burgundy", "champagne": "gold", "charcoal": "gray",
    }
    for keyword, canonical in color_map.items():
        if keyword in v: return canonical
    if v in ("custom", "other"): return v
...
def extract_title(rec: Dict[str, Any], text: str) -> str:
    ...
    if header:
        ...
        if clean.strip(): return clean.strip()
        return header
    ...
    if isinstance(meta, dict):
        for k in ("title", "page_title", "document_title"):
            ...
            if isinstance(v, str) and v.strip():
                return re.sub(r"\s*-\s*craigslist\s*$", "", v.strip(), flags=re.I)
...
def extract_location(rec: Dict[str, Any], text: str, attrs: Dict[str, str]) -> Optional[str]:
    # 1. Location in title parentheses: "2020 Honda Accord (Sacramento)"
    header = extract_cl_header(text)
    if header:
        ...
        if loc_match:
            ...
            noise = [
                ...
                "euro auto", "we finance", "availability", "510-", "408-", "over 100",
            ]
            if not any(n in loc.lower() for n in noise):
                return loc

    # 2. CL attribute block location field
    loc_attr = attrs.get("location")
    if loc_attr:
        return strip_markdown_links(loc_attr)

    # 3. Tavily page title (only present with old Tavily runs)
    meta = rec.get("tavily_meta") or {}
    page_title = ""
    if isinstance(meta, dict):
        page_title = meta.get("title") or meta.get("page_title") or ""
    city_match = re.search(r"-\s*([^-]+?),\s*[A-Z]{2}\s*-\s*craigslist", page_title, re.I)
    if city_match:
        return city_match.group(1).strip()

    # 4. Subdomain from URL: sacramento.craigslist.org → "sacramento"
    url = rec.get("url") or ""
    subdomain = re.match(r"https?://([^.]+)\.craigslist\.org", url)
    if subdomain:
        region = subdomain.group(1)
        if region not in ("www", "craigslist"):
            return region
...
def first_int_from_miles(text: str) -> Optional[int]:
    for m in MILES_RE.finditer(text or ""):
        ...
        if " or " in pre and any(w in pre for w in ("day", "hour", "week", "month")):
            continue
        try:
            val = int(m.group(1).replace(",", ""))
            if 100 <= val <= 500000: return val
        except ValueError:
            continue
    return None
...
def parse_odometer(attrs: Dict[str, str]) -> Optional[int]:
    raw = strip_markdown_links(attrs.get("odometer", "").strip())
...
def phone_numbers(text: str, post_id: str = "") -> List[str]:
    ...
    for n in nums:
        ...
        if post_id and digits_only == post_id: continue
        if re.match(r"^791\d{7}$", digits_only): continue
        if n2 not in seen:
            seen.add(n2)
            cleaned.append(n2)
    return cleaned

def extract_images(rec: Dict[str, Any], text: str) -> List[str]:
    # Prefer pre-extracted image_urls from Stage 3 (BS4 scraper)
    pre = rec.get("image_urls")
    if pre and isinstance(pre, list) and len(pre) > 0:
        return pre

    # Fallback: scan raw_content for image URLs (Tavily runs)
    urls = IMG_URL_RE.findall(text or "")
    seen: set = set()
    out = []
    for u in urls:
        if u in seen or "50x50" in u: continue
        seen.add(u)
        out.append(u)
    return out
...
def build_record(rec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Year fallback: scan full page text if title had no year
    if not year:
    ...
    images      = extract_images(rec, text)
    ...
    return {
        ...
        "parsed_at":           now_iso(),
        "tavily_meta":         (rec.get("tavily_meta")
                                if isinstance(rec.get("tavily_meta"), dict) else None),
    }
...
def main() -> int:
    ...
    structured = [build_record(rec) for rec in raw_recs]
